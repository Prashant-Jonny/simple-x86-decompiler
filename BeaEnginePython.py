...
if os.name == 'nt':
   try:
     __module = WinDLL('BeaEngine64')
   except:
      __module = WinDLL('BeaEngine32')
elif os.name == 'posix':
...
def opcode2asm(s_,virtual_addr):
   bit=32
   s = create_string_buffer(s_,len(s_))
   Instruction = DISASM()
   Instruction.EIP = addressof(s)
   Instruction.VirtualAddr = virtual_addr
   Instruction.Options = Tabulation + NasmSyntax + SuffixedNumeral  + ShowSegmentRegs
   while Instruction.EIP < addressof(s)+len(s_):
      InstrLength = Disasm(addressof(Instruction))
      Instruction.EIP+=InstrLength
      yield Instruction,InstrLength
